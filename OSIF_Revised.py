...
class OSIF:
    def __init__(self, master):
        ...
        # NEW: Add an attribute to track the best (lowest) cost.
        ...
        self.fitB.grid(row=0, column=2, sticky=E)

        self.simB = Button(buttonFrame, text="Simulate", command=self.PerformSim)
        self.simB.grid(row=0, column=1, sticky=W)
        ...
        self.autoFreqButton.grid(row=0, column=3, sticky=W)

        # Add the "Update Parameters" button.
        self.updateParamsButton = Button(buttonFrame, text="Update parameters", command=self.update_parameters)
        self.updateParamsButton.grid(row=0, column=4, sticky=W)
    ...
    def LoadSElectedFile(self):
        ...
        # Estimate Rmem from the high-frequency intercept (minimum real part)
        if len(self.activeData.rawzPrime) > 0:
            est_rmem = min(self.activeData.rawzPrime)
    ...
    def fit_chopped_window(self, upper_idx, lower_idx, chopped_freq, N_initial):
        ...
        # ------------------------------------------------------------------
        # New Helper Method: get_fit_cost_and_params
        # This method performs a free‐parameter fit (using Rmem, Rcl, Qdl, and Phi from the IE fields)
        # on the currently chopped data and returns both the cost (for information) and
        # the optimized free parameter vector.
    ...
    def get_fit_cost_and_params(self):
        ...
        # (Optional: Calculate cost from the Jacobian and uncertainties if desired)
        J = result.jac
        try:
            cov = np.linalg.inv(J.T @ J)
        except np.linalg.LinAlgError:
            cov = np.linalg.pinv(J.T @ J)
        dof = len(result.fun) - len(free_params)
        s2 = (result.fun.T @ result.fun) / dof if dof > 0 else 0
        estVars = np.diag(cov * s2)
        std = np.sqrt(estVars)
    ...
    def auto_freq_window_grid_search(self):
        """
        Grid search for the optimal frequency window.

        Steps:
          1. Chop the data based on the current frequency bounds.
          2. Perform a grid search for the best candidate frequency window.
          3. Update the frequency bound fields accordingly.
          4. Re-chop the data based on these new bounds.
        ...
        if npts < 10:
            tkMessageBox.showinfo("Auto-Freq Error", "Not enough data points in current frequency window.")
            return
        N_initial = npts

        # Step 2: Define candidate indices.
        ...
        for ui in cand_upper_indices:
            for li in cand_lower_indices:
                ...
                print("Candidate window: {:.2f} Hz to {:.2f} Hz, cost: {:.2f}%"
                      .format(chopped_freq[ui], chopped_freq[li], candidate_cost))
        ...
        # Step 3: Update the frequency bounds (IE and OE fields).
        optimal_upper = chopped_freq[best_upper_idx]
        optimal_lower = chopped_freq[best_lower_idx]
        self.frequencyRange.IE.delete(0, END)
        self.frequencyRange.IE.insert(0, "{:.2f}".format(optimal_upper))
        self.frequencyRange.OE.delete(0, END)
        self.frequencyRange.OE.insert(0, "{:.2f}".format(optimal_lower))
        print("Optimal window: {:.2f} Hz to {:.2f} Hz (cost: {:.2f}%)."
    ...
    def update_parameters(self):
        ...
        try:
            # Rmem
            new_Rmem = self.Rmem.OE.get()
            print("Updating Rmem: reading from OE =", new_Rmem)
            self.Rmem.IE.config(state="normal")  # ensure field is writable
            self.Rmem.IE.delete(0, END)
            self.Rmem.IE.insert(0, new_Rmem)

            # Rcl
            new_Rcl = self.Rcl.OE.get()
            print("Updating Rcl: reading from OE =", new_Rcl)
            self.Rcl.IE.config(state="normal")
            self.Rcl.IE.delete(0, END)
            self.Rcl.IE.insert(0, new_Rcl)

            # Qdl
            new_Qdl = self.Qdl.OE.get()
            print("Updating Qdl: reading from OE =", new_Qdl)
            self.Qdl.IE.config(state="normal")
            self.Qdl.IE.delete(0, END)
            self.Qdl.IE.insert(0, new_Qdl)

            # Phi
            new_Phi = self.Phi.OE.get()
            print("Updating Phi: reading from OE =", new_Phi)
            ...
            tkMessageBox.showerror("Update Parameters Error", str(e))
